...
type TextMultiInputCommandRequest interface {
	Name() string
	inputs() []CommandInput
	text() string
	options() []string
...
func OptionsForTextMultiInput(request TextMultiInputCommandRequest, token token, cm ContextManager, file func(input CommandInput) string) []string {
	...
	options = append(options, request.text())
...
func RunCommandForTextMultiInput(request TextMultiInputCommandRequest, tg TokenGenerator, cm *ContextManager) Response {
	argumentsForCommand := OptionsForTextMultiInput(request, request.token(), *cm, func(input CommandInput) string {
		if input.File != nil {
			return *input.File
		}
		if input.Pipe != nil {
	...
	argumentsForShortCommand := OptionsForTextMultiInput(request, request.token(), *cm, func(input CommandInput) string {
		if input.File != nil {
	...
	arguments := OptionsForTextMultiInput(request, request.token(), *cm, func(input CommandInput) string {
